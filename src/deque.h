...
// #define deque_init(arr) (Deque){ .items = (arr), .len = arr_size((arr)), .capacity = arr_size((arr)) .front = 0 }
#define deque_init() (Deque){0}
...
#define deque_get_back(deque) ((deque)->front + (deque)->len) % (deque)->capacity
...
do {\
    ...
    if (i != NULL) {\
        /* ensures that -1 wraps around to ((deque)->capacity - 1) */ \
        *i = (deque)->items[(deque_get_back((deque)) - 1 + (deque)->capacity) %\
                         (deque)->capacity];\
    }\
} while (0)
